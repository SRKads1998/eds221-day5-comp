...
# Create time series of years to estimate pop size.
time_yr <- seq(from = 0, to = 8, by = 1)

# Create empty matrix to store pop sizes
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# Populating first row with year 0 pop sizes.
proj_year[1, ] <- fish_year0

# Estimate pop size for each year base on pop size previous year
for (i in 2:length(time_yr)) { #start w/ year 1
  proj_year[i,] <- fish_leslie %*% proj_year[i-1,]
}


# The rest is wrangling & visualization (run to check it out):
# updating col names for pop states
